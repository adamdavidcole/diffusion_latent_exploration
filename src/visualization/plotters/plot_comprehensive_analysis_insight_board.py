...
def plot_comprehensive_analysis_insight_board(
    ...
    """Publication board with clear hierarchy and CIs on key bars."""
    ...
    name_cmap = getattr(viz_config, 'name_cmap', 'tab10')
    ...
    def norm01(a):
        ...
        d = (M - m) if (M - m) > 1e-12 else 1.0
        return (a - m) / d

    # def corr_vs_rung(y):
    #     y = np.asarray(y, float)
    #     x = np.arange(len(y), float)
    ...
    def spatial_curve(full_res, g):
        spg = full_res.spatial_patterns['trajectory_spatial_evolution'].get(g, {})
        for k in ('spatial_variance_curve', 'spatial_variance_by_step', 'variance_curve'):
            if k in spg: 
                arr = np.asarray(spg[k], float)
    ...
    for c, g, lab in zip(cols, groups, labels):
        y = spatial_curve(results_full, g)
        if y is not None:
    ...
    # choose one: late/early OR geometry; geometry is often more illustrative
