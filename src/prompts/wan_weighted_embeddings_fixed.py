"""
WAN-specific weighted embeddings implementation - FIXED VERSION
...
class WANWeightedEmbeddings:
    ...
    def _create_weighted_embeddings_t5(
        ...
        input_ids = text_inputs.input_ids[0]  # [max_sequence_length]
        attention_mask = text_inputs.attention_mask[0]  # [max_sequence_length]
        seq_len = attention_mask.sum().item()
        
        logger.info(f"Full sequence length: {seq_len}, Max length: {max_sequence_length}")
        
        # Get base embeddings - these will be [1, max_sequence_length, hidden_dim]
        with torch.no_grad():
        ...
        # Create weight map for each token position - must match embedding sequence length
        token_weights = torch.ones(base_embeddings.size(1), device=self.device, dtype=base_embeddings.dtype)
    ...
    def _apply_token_weights(self, embeddings: torch.Tensor, weights: torch.Tensor, method: str = "interpolation") -> torch.Tensor:
        ...
        """
        logger.info(f"Applying weights using method: {method}")
        logger.info(f"Embeddings shape: {embeddings.shape}, Weights shape: {weights.shape}")
        ...
        if method == "multiply":
            ...
            # Find the EOS token - it's typically the first padding token or last real token
            eos_embedding = embeddings[0, -1:, :]  # [1, hidden_dim] - last token as fallback
            
            # Try to find actual EOS token
            eos_token_id = self.tokenizer.eos_token_id
            if eos_token_id is not None:
                # Find where EOS token appears in the input
                text_inputs = self.tokenizer(
                    [""],  # Empty to find just EOS
                    padding="max_length",
                ...
                # Use the embedding at position where EOS would be for this specific sequence
                eos_pos = min(weights.nonzero().max().item() + 1, embeddings.size(1) - 1)
                eos_embedding = embeddings[0, eos_pos:eos_pos+1, :]  # [1, hidden_dim]
...
def create_wan_weighted_embeddings(pipe, prompt: str, max_sequence_length: int = 512, weighting_method: str = "multiply") -> torch.Tensor:
    """
